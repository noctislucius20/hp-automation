...
def create_honeypotsensor():
    ...
    try:
        ...
        new_honeypotsensor = HoneypotSensorService().add_honeypotsensor(honeypot=data.get('honeypot'), sensor_id=data.get('sensor_id'))

        response = make_response({'status': 'success', 'message': 'new honeypotsensor created', 'data': new_honeypotsensor[0]})
